<script>
  ...
  let translate = 0;
  let listSize = 160 * developers.length + 80 * (developers.length - 1);
  const next = () => {
    if( Math.abs(translate) >= listSize ) return;
    translate -= 240;
  }
  const prev = () => {
...
<div class="developers-list">
  ...
  <svg class="left-arrow" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" on:click={prev}>
  ...
  <div class="developers-wrapper">
    <ul class="developers" style={`transform: translateX(${translate}px);`}>
      {#each developers as developer (developer.id) }
      <a class="developer" href={developer.link}>
        <img class="developer-logo" src={developer.img} alt="">
      </a>
      {/each}
    </ul>
  </div>

  <svg class="right-arrow" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" on:click={next}>
